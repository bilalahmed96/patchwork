...
class CompanyController extends AdminController
{
    ...
    public function create(Request $request)
    {
        ...
        $data = [
         ...
         if(!is_admin())
        {
          return  view('Company::admin.company.create', $data);
    ...
    public function edit(Request $request, $id)
    {
        ...
        if(!is_admin()){
            return view('Company::admin.company.edit', $data); 
            // return view('Company::admin.company.detail', $data);
        }
        return view('Company::admin.company.detail', $data);
    }
    ...
    public function store(Request $request, $id){
        ...
        if($id>0){
            ...
            if(!is_admin())
            {
                ...
                $row->avatar_id = MediaFile::saveUploadFile($request->avatar_id);
                if(! empty($request->admin_features)){
                    $row->admin_features = implode('!', $request->admin_features);
                }
            }
        }
        $attr = [
    ...
    public function my_companies(Request $request){
          $this->checkPermission('employer_manage');
        if(!is_admin())
        {
        $dataCompany = $this->company::query()->where('owner_id',Auth::id())->orderBy('id', 'desc');
        $company_name = $request->query('s');
        $cate = $request->query('category_id');
        if ($cate) {
            $dataCompany->where('category_id', $cate);
        ...
        return view('Company::admin.company.my-company', $data);
        }
    }
