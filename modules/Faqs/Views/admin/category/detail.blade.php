...
@section('content')
    <form action="{{route('faqs.admin.category.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        @csrf
        <div class="container">
                ...
                <div class="">
                    ...
                    @if($row->slug)
                        <p class="item-url-demo"> {{ __('Permalink:')}}
                            {{ url((request()->query('lang') ? request()->query('lang').'/' : '').config('faqs.faqs_route_prefix')."/".config('faqs.faqs_category_route_prefix')) }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
                    ...
                    <div class="col-md-9">
                        <div class="panel">
                            <div class="panel-body">
                                ...
                                @include('Faqs::admin/category/form')
                            </div>
                        </div>
