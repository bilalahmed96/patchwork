...
class PlanController extends FrontendController
{
    ...
    public function single_package(){
        ...
        $data = [
            'page_title'=>__('Single Package'),
            'plans'=>$role->plans,
            'user'=>auth()->user(),
        ];
        return view("User::frontend.plan.single",$data);
    }  
       ...
       public function monthly_package(){
        ...
        $data = [
            'page_title'=>__('Monthly Package'),
            'plans'=>$role->plans,
            'user'=>auth()->user(),
        ];
        return view("User::frontend.plan.monthly",$data);
    }  
    
       public function annual_package(){
         if(auth()->check()){
            $role = auth()->user()->role;
        }else{
            $role = Role::find('employer');
        ...
        $data = [
            'page_title'=>__('Annual Package'),
            'plans'=>$role->plans,
            'user'=>auth()->user(),
        ];
        return view("User::frontend.plan.annual",$data);
    }  
}
