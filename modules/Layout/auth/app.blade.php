    ...
    $(document).ready(function(){
      ...
      $(document).on("click",".img-btn",function(){
        ...
        if($(this).hasClass('_1')){
            ...
            $('._2').closest('div').find('.account-type-radio').attr('checked',false);
            
        } else if($(this).hasClass('_2')){
            $(this).closest('div').find('.account-type-radio').attr('checked',true); 
            $('._1').closest('div').find('.account-type-radio').attr('checked',false);
            
        }
        $(this).attr('style', 'margin-bottom: 10px;margin-top: 10px;box-shadow: rgb(0, 0, 0) 0px 10px 27px -21px;border-radius: 20px;border-right: 10px solid rgb(114, 41, 99);border-bottom: 11px solid rgb(114, 41, 99)')
        $('.img-btn').not(this).each(function(){ $(this).css({'box-shadow': 'none', 'border-radius': 'none', 'border-right': 'none ', 'border-bottom': 'none'}) });
    });

     $(document).on('click' , '.btn-show' , function(){
      console.log('pass')
      console.log($(this).siblings('input').attr('type') == "password");
