...
Route::group(['prefix'=>config('job.job_route_prefix')],function(){
    ...
    Route::get('/post-job','JobController@create')->name('job.post');
    Route::post('/post-job','JobController@store')->name('job.post');
    ...
    Route::get('/{cat_slug}/{location_slug}', 'JobController@categoryLocationIndex')->name('job.location.index');
});
